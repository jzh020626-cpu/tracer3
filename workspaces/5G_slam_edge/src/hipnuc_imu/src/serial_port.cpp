...
class IMUPublisher : public rclcpp::Node
{
	public:
		...
		IMUPublisher() : Node("tracer3_IMU_publisher")	
		{
			...
			yaw_offset_rad = yaw_offset_deg * DEG_TO_RAD;
			...
			RCLCPP_INFO(this->get_logger(), "yaw_offset_deg: %.6f\r\n", yaw_offset_deg);
	...
	private: 
		...
		void apply_yaw_offset(sensor_msgs::msg::Imu &msg)
		{
			if(std::abs(yaw_offset_rad) < 1e-12)
				return;

			const double half_yaw = yaw_offset_rad * 0.5;
			const double cw = std::cos(half_yaw);
			const double cz = std::sin(half_yaw);
			...
			msg.orientation.w = cw * w - cz * z;
			msg.orientation.x = cw * x - cz * y;
			msg.orientation.y = cw * y + cz * x;
			msg.orientation.z = cw * z + cz * w;
		}
